...
test.describe('Sistema de Registro de Ponto', () => {
  ...
  test('deve exibir a interface de registro de ponto', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Portal do Estagiário').or(page.locator('text=Painel do Funcionário'))
    ).toBeVisible()

    // Verificar se o botão de registrar ponto está visível
    const registerButton = page
      .locator('text=Registrar Entrada')
      .or(page.locator('text=Registrar Saída'))
      .or(page.locator('text=Abrir Scanner'))
  ...
  test('deve abrir o modal do scanner quando clicar em registrar ponto', async ({ page }) => {
    ...
    if (await registerButton.first().isVisible()) {
      ...
      await expect(
        page.locator('text=Registrar Ponto').or(page.locator('text=Scanner'))
      ).toBeVisible()

      // Verificar se há instruções para o usuário
      await expect(
        page.locator('text=Aponte a câmera').or(page.locator('text=câmera'))
      ).toBeVisible()
      ...
      const closeButton = page
        ...
        .or(page.locator('text=Cancelar'))
        .or(page.locator('text=Fechar'))
  ...
  test('deve simular registro de ponto com QR code', async ({ page }) => {
    ...
    if (await registerButton.first().isVisible()) {
      ...
      await expect(page.locator('text=registrada').or(page.locator('text=sucesso'))).toBeVisible({
        timeout: 10000,
      })
  ...
  test('deve exibir histórico de registros', async ({ page }) => {
    ...
    const historySection = page
      .locator('text=Registros Recentes')
      .or(page.locator('text=Histórico'))
      .or(page.locator('text=Últimos Registros'))
    ...
    if (await historySection.isVisible()) {
      ...
      const records = page
        .locator('[data-testid="attendance-record"]')
        .or(page.locator('.record-item'))
        .or(page.locator('text=Entrada').or(page.locator('text=Saída')))
