...
test.describe('Teste de Interface do Sistema de Ponto', () => {
  ...
  test('deve tentar acessar página do funcionário e ser redirecionado para login', async ({
    page,
  }) => {
    await page.goto('http://localhost:3000')
  ...
  test('deve verificar responsividade da página inicial', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      console.log(
        `✅ Layout responsivo funcionando em ${viewport.name} (${viewport.width}x${viewport.height})`
      )
    }
  })
