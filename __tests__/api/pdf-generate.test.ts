...
jest.mock('@/lib/pdf-schemas/templates', () => ({
  ...
  buildProfessionalDeclarationSchema: jest
    .fn()
    .mockReturnValue({ title: 'Professional Declaration' }),
  buildInternshipRegistrationSchema: jest
    .fn()
    .mockReturnValue({ title: 'Internship Registration' }),
  buildInternshipRegistrationRequestSchema: jest
    .fn()
    .mockReturnValue({ title: 'Registration Request' }),
...
describe('API /api/pdf/generate', () => {
  ...
  describe('POST - Generate PDF', () => {
    ...
    it('deve retornar erro 500 quando geração do PDF falha', async () => {
      ...
      ;(generatePDFFromSchema as jest.Mock).mockRejectedValueOnce(
