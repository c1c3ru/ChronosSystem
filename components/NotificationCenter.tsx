    ...
    return (
        <div className="space-y-3 mb-6">
            {notifications.map((n) => (
                <div
                    key={n.id}
                    className={`relative overflow-hidden rounded-xl border p-4 shadow-lg transition-all animate-in slide-in-from-top-2 duration-300 ${n.type === 'MISSED_EXIT'
                ...
                >
                    <div className="flex items-start justify-between">
                        <div className="flex items-start space-x-3">
                            <div className={`mt-0.5 rounded-full p-2 ${n.type === 'MISSED_EXIT' ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/20 text-amber-400'
                                }`}>
                                {n.type === 'MISSED_EXIT' ? <AlertTriangle size={18} /> : <Clock size={18} />}
                            </div>
                            <div>
                                <h4 className={`font-bold ${n.type === 'MISSED_EXIT' ? 'text-red-400' : 'text-amber-400'}`}>
                    ...
                    <div className={`absolute bottom-0 left-0 h-1 bg-current opacity-20 transition-all w-full ${n.type === 'MISSED_EXIT' ? 'text-red-500' : 'text-amber-500'
                        }`} />
                </div>
            ))}
        </div>
