...
export async function buildEquivalenceRequestDoc(d: EquivalenceRequestData): Promise<TDocumentDefinitions> {
  ...
  const studentTable = dataTable(
    ...
    [
      ...
      [cell('ENDEREÇO (LOGRADOURO, NÚMERO E COMPLEMENTO)', v(d.student_address)), cell('BAIRRO/DISTRITO', v(d.student_neighborhood))],
      [
        {
          columns: [
            cell('MUNICÍPIO-UF', v(d.student_city_uf), { width: '40%', border: [false, false, true, true] }),
      ...
      [cell('E-MAIL INSTITUCIONAL', v(d.student_email_inst)), cell('E-MAIL PESSOAL', v(d.student_email_personal))],
    ]
  )
  ...
  const personalInfoTable = dataTable(
    ...
    [
      [
        ...
        { text: 'APENAS PARA PESSOA COM DEFICIÊNCIA', style: 'tableHeader', alignment: 'center' as Alignment },
      ],
      [
        {
          stack: [
        ...
        },
        {
          stack: [
            ...
            { text: `Informar comunidade se marcar etnia: ${v(d.ethnicity_community)}`, fontSize: 6, margin: [0, 4, 0, 0] },
          ],
          margin: [2, 2, 2, 2]
        },
        {
          stack: [
            { text: `${cb(d.disability?.includes('alta_habilidade'))} Alta habilidade/superdotação`, fontSize: 7 },
            { text: `${cb(d.disability?.includes('auditiva'))} Deficiência auditiva`, fontSize: 7 },
            { text: `${cb(d.disability?.includes('intelectual'))} Deficiência intelectual`, fontSize: 7 },
            { text: `${cb(d.disability?.includes('motora'))} Deficiência motora`, fontSize: 7 },
            { text: `${cb(d.disability?.includes('visual_baixa'))} Deficiência visual/baixa visão`, fontSize: 7 },
  ...
  const experienceTable = dataTable(['*'], [
    [{ text: 'EXPERIÊNCIA A SER APROVEITADA COMO ATIVIDADE DE ESTÁGIO', style: 'tableHeader', alignment: 'center' as Alignment }],
    [{
      stack: [
        { text: `${cb(d.exp_extension)} Atividade de extensão, Iniciação científica ou monitoria`, fontSize: 8, margin: [0, 2, 0, 2] },
        { text: `${cb(d.exp_employee)} Empregado de empresa privada ou pública`, fontSize: 8, margin: [0, 2, 0, 2] },
        { text: `${cb(d.exp_third_sector)} Membro ou empregado de instituição do terceiro setor`, fontSize: 8, margin: [0, 2, 0, 2] },
        { text: `${cb(d.exp_public_servant)} Servidor público estatutário`, fontSize: 8, margin: [0, 2, 0, 2] },
      ],
      margin: [5, 5, 5, 5]
    }]
  ])
  ...
  const docsTable = dataTable(['*'], [
    ...
    [{
      stack: [
        { text: `${cb(d.doc_ata_third_sector)} Ata de nomeação do membro de instituição do terceiro setor`, fontSize: 8, margin: [0, 1, 0, 1] },
        { text: `${cb(d.doc_nomination_public)} Ato de nomeação do servidor público`, fontSize: 8, margin: [0, 1, 0, 1] },
        { text: `${cb(d.doc_cnpj)} Cartão do Cadastro Nacional de Pessoa Jurídica – CNPJ`, fontSize: 8, margin: [0, 1, 0, 1] },
        { text: `${cb(d.doc_ctps)} Carteira de Trabalho e Previdência Social – CPTS`, fontSize: 8, margin: [0, 1, 0, 1] },
        { text: `${cb(d.doc_statute)} Contrato Social ou Estatuto da instituição`, fontSize: 8, margin: [0, 1, 0, 1] },
        { text: `${cb(d.doc_activities_declaration)} Declaração de atividades`, fontSize: 8, margin: [0, 1, 0, 1] },
        { text: `${cb(d.doc_public_functions)} Regulamento das funções do cargo público`, fontSize: 8, margin: [0, 1, 0, 1] },
        { text: `${cb(d.doc_others)} Outros documentos: ${v(d.doc_others_desc)}`, fontSize: 8, margin: [0, 1, 0, 1] },
      ],
      margin: [5, 5, 5, 5]
    }]
  ])
  ...
  const internshipTable = dataTable(
    ...
    [
      ...
      [
        { text: 'OBRIGATÓRIO', fontSize: 9, bold: true, alignment: 'center' as Alignment, margin: [0, 8, 0, 0] },
        {
          stack: [
            { text: `${cb(d.internship_mode === 'presencial')} PRESENCIAL`, fontSize: 7, margin: [0, 2, 0, 2] },
            { text: `${cb(d.internship_mode === 'remoto')} REMOTO`, fontSize: 7, margin: [0, 2, 0, 2] },
          ],
          margin: [5, 2, 5, 2]
        },
        { text: fmtDate(d.start_date), fontSize: 9, alignment: 'center' as Alignment, margin: [0, 8, 0, 0] },
        { text: `${v(d.weekly_hours)} HORAS`, fontSize: 9, alignment: 'center' as Alignment, margin: [0, 8, 0, 0] },
        { text: fmtDate(d.end_date_expected), fontSize: 9, alignment: 'center' as Alignment, margin: [0, 8, 0, 0] },
      ]
    ]
  )
  ...
  const scheduleRows: TableCell[][] = [
    [
      ...
      ...dayLabels.map(label => ({ text: label, style: 'tableHeader', alignment: 'center' as Alignment, colSpan: 2 })),
  ...
  const scheduleTable = dataTable(
    ['4.7%', ...Array(14).fill('6.8%')],
    scheduleRows
  )

  const content: Content[] = [
    ...header,
    { text: 'SOLICITAÇÃO DE APROVEITAMENTO DE EXPERIÊNCIA ACADÊMICA OU DE TRABALHO', style: 'docTitle', margin: [0, 10, 0, 10] },
    studentTable,
    { text: '\n', fontSize: 4 },
    personalInfoTable,
    { text: '\n', fontSize: 4 },
    ...
    { text: '\n', fontSize: 4 },
    internshipTable,
    { text: 'PREVISÃO DE DISTRIBUIÇÃO DA CARGA HORÁRIA', style: 'tableHeader', alignment: 'center' as Alignment, margin: [0, 2, 0, 0] },
    scheduleTable,
    { text: '\n' },
    {
      columns: [
        {
          stack: [
            { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 200, y2: 0, lineWidth: 0.5 }], margin: [0, 20, 0, 2] },
        ...
        {
          stack: [
            { text: `${v(d.city_uf || 'Maracanaú-CE')}, ${fmtDate(d.solicitation_date)}`, fontSize: 9, alignment: 'right' as Alignment, margin: [0, 20, 0, 0] }
          ],
          width: '50%'
        }
      ]
