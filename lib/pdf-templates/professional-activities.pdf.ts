...
export async function buildProfessionalActivitiesDoc(d: ProfessionalActivitiesData): Promise<TDocumentDefinitions> {
  const header = await ifceHeader()

  const companyTable = dataTable(['70%', '30%'], [
    [{ text: 'DADOS DA EMPRESA', style: 'tableHeader', alignment: 'center' as Alignment, colSpan: 2 }, emptyCell()],
    [cell('RAZÃO SOCIAL', v(d.company_name)), cell('CNPJ', v(d.company_cnpj))],
    [cell('ENDEREÇO', v(d.company_address), { colSpan: 2 }), emptyCell()]
  ])

  const employeeTable = dataTable(['70%', '30%'], [
    [{ text: 'DADOS DO FUNCIONÁRIO', style: 'tableHeader', alignment: 'center' as Alignment, colSpan: 2 }, emptyCell()],
  ...
  const contractTable = dataTable(['33%', '34%', '33%'], [
    [{ text: 'DADOS DO VÍNCULO', style: 'tableHeader', alignment: 'center' as Alignment, colSpan: 3 }, emptyCell(), emptyCell()],
    [
  ...
  const activitiesTable = dataTable(['*'], [
    [{ text: 'DESCRIÇÃO DAS ATIVIDADES', style: 'tableHeader', alignment: 'center' as Alignment }],
    [{
      text: v(d.activities),
      fontSize: 9,
      margin: [5, 5, 5, 30]
  ...
  const content: Content[] = [
    ...
    },
    {
      table: {
        widths: ['*', '*'],
        body: [[
          { text: [{ text: 'SOLICITAÇÃO EM ', bold: true, fontSize: 7 }, { text: fmtDate(d.solicitation_date), fontSize: 7 }], border: [true, true, false, true], margin: [4, 3, 4, 3] },
          { text: [{ text: 'AUTORIZAÇÃO EM ', bold: true, fontSize: 7 }, { text: fmtDate(d.authorization_date), fontSize: 7 }], border: [false, true, true, true], margin: [4, 3, 4, 3] },
        ]],
      },
      layout: {
        hLineWidth: () => 0.5,
