...
export async function buildExperienceDeclarationDoc(
  d: ExperienceDeclarationData
): Promise<TDocumentDefinitions> {
  const header = await ifceHeader()
  const cb = (checked?: boolean) => (checked ? '( X )' : '(   )')

  const declarantTable = dataTable(
    ['75%', '25%'],
    [
      [
        cell(
          'NOME DO DECLARANTE (SERVIDOR(A) ORIENTADOR(A)/SUPERVISOR(A) DA BOLSA OU COORDENADOR(A) DO PROJETO/PROGRAMA)',
          v(d.declarant_name),
          { colSpan: 2 }
        ),
        emptyCell(),
      ...
      [
        {
          columns: [
            {
              text: `${cb(d.exp_type === 'extensao')} EXTENSÃO`,
              fontSize: 9,
              alignment: 'center',
              width: '33%',
            },
            {
              text: `${cb(d.exp_type === 'iniciacao')} INICIAÇÃO CIENTÍFICA`,
              fontSize: 9,
              alignment: 'center',
              width: '33%',
            },
            {
              text: `${cb(d.exp_type === 'monitoria')} MONITORIA`,
              fontSize: 9,
              alignment: 'center',
              width: '34%',
            },
  ...
  const content: Content[] = [
    ...header,
    {
      text: 'DECLARAÇÃO DE PARTICIPAÇÃO EM EXPERIÊNCIA DE EXTENSÃO, INICIAÇÃO CIENTÍFICA OU MONITORIA',
      style: 'docTitle',
      margin: [0, 10, 0, 10],
    },
    declarantTable,
