...
export async function buildFinalReportDoc(d: FinalReportData): Promise<TDocumentDefinitions> {
  ...
  const capa: Content[] = [
    {
      text: v(d.student_name).toUpperCase(),
    ...
    {
      text: 'RELATÓRIO FINAL DE ESTÁGIO OBRIGATÓRIO',
      alignment: 'center' as Alignment,
      fontSize: 14,
      bold: true,
      margin: [0, 0, 0, 150],
    ...
    {
      text: `${v(d.local).toUpperCase()} / ${v(d.ano)}`,
      alignment: 'center' as Alignment,
      fontSize: 12,
      bold: true,
      pageBreak: 'after',
  ...
  const resumoPage: Content[] = [
    ...
    {
      text: `Palavras-chaves: ${v(d.palavras_chave)}`,
      fontSize: 11,
      bold: true,
      pageBreak: 'after',
    },
  ...
  const dataTableEstagio = dataTable(
    ['20%', '20%', '20%', '20%', '20%'],
    [
      [
      ...
      [
        ...
        },
        {
          stack: [
            ...
            { text: `${cb(d.internship_mode === 'virtual')} VIRTUAL`, fontSize: 7 },
          ],
          margin: [2, 2, 2, 2],
        },
        {
  ...
  const activitiesLeveragedTable = dataTable(
    ...
    [
      [
        {
          text: 'ATIVIDADES DO ESTÁGIO APROVEITADAS',
          style: 'tableHeader',
          alignment: 'center' as Alignment,
        },
      ],
      [
        {
          columns: [
            {
              stack: [
                { text: `${cb(d.activities_leveraged === 'extensao')} EXTENSÃO`, fontSize: 7 },
                {
                  text: `${cb(d.activities_leveraged === 'ic')} INICIAÇÃO CIENTÍFICA`,
                  fontSize: 7,
                },
              ],
              width: '33%',
            ...
            {
              stack: [
                { text: `${cb(d.activities_leveraged === 'monitoria')} MONITORIA`, fontSize: 7 },
                {
                  text: `${cb(d.activities_leveraged === 'terceiro_setor')} MEMBRO DE INSTITUIÇÃO DO 3º SETOR`,
                  fontSize: 7,
                },
              ],
              width: '33%',
            },
            {
              stack: [
                {
                  text: `${cb(d.activities_leveraged === 'trabalho_clt')} TRABALHO FORMAL CELETISTA`,
                  fontSize: 7,
                },
                {
                  text: `${cb(d.activities_leveraged === 'trabalho_estatutario')} TRABALHO FORMAL ESTATUTÁRIO`,
                  fontSize: 7,
                },
                {
                  text: `${cb(d.activities_leveraged === 'nao_se_aplica')} NÃO SE APLICA`,
                  fontSize: 7,
                },
              ],
              width: '34%',
  ...
  const buildEvalRows = (items: EvalItem[], data: Record<string, string>) =>
    items.map((item) => [
  ...
  const supervisorEvalTable = dataTable(
    ['40%', '15%', '15%', '15%', '15%'],
    [
      evalHeaders.map((h) => ({ text: h, style: 'tableHeader', alignment: 'center' as Alignment })),
  ...
  const generalEvalTable = dataTable(
    ['25%', '25%', '25%', '25%'],
    [
      [
  ...
  const content: Content[] = [
    ...
    ...sumarioPage,
    {
      text: '1. INTRODUÇÃO: Identificação dos atores e entes envolvidos no estágio',
      style: 'sectionTitle',
      margin: [0, 0, 0, 10],
    },
    studentTable,
    ...
    autoEvalTable,
    {
      text: 'COMPLEMENTAR AUTO AVALIAÇÃO ATRAVÉS DE ASPECTOS DO APRENDIZADO',
      style: 'tableHeader',
      margin: [0, 10, 0, 5],
    },
    ...
    {
      text: 'AVALIAÇÃO DA SUPERVISÃO DO ESTÁGIO',
      style: 'tableHeader',
      alignment: 'center' as Alignment,
      margin: [0, 5, 0, 5],
    },
    ...
    {
      text: 'AVALIAÇÃO GERAL SOBRE O ESTÁGIO',
      style: 'tableHeader',
      alignment: 'center' as Alignment,
      margin: [0, 10, 0, 5],
    },
    ...
    {
      text: '4.2. Indicação de Áreas de Conhecimento:',
      bold: true,
      fontSize: 11,
      margin: [0, 0, 0, 10],
    },
    {
      text: v(d.consideracoes_matriz),
      alignment: 'justify' as Alignment,
      fontSize: 11,
      lineHeight: 1.5,
    },
    ...
    { text: '\n\n' },
    {
      text: `${v(d.local)}-CE, ______de ____________________de 20____.`,
      alignment: 'center' as Alignment,
      margin: [0, 50, 0, 50],
    },
    {
