 ...
 */
export function validateEntryTime(entryTime: Date, config: UserShiftConfig): ShiftValidationResult {
  const expectedDailyHours = calculateExpectedDailyHours(
    config.weeklyHours,
    config.workingDaysPerWeek
  )
  const allowedEntryWindow = getAllowedEntryWindow(
    config.shift,
    config.shiftStartTime,
    config.shiftEndTime
  )
  const allowedExitWindow = getAllowedExitWindow(
    config.shift,
    config.shiftStartTime,
    config.shiftEndTime,
    expectedDailyHours
  ...
  const isWithinWindow = isTimeWithinWindow(
    entryTimeStr,
    allowedEntryWindow.start,
    allowedEntryWindow.end
  )
  ...
  if (entryTime.getHours() < SECTOR_HOURS.primary.start) {
    warnings.push(
      `Entrada antes do horário de funcionamento do setor (${SECTOR_HOURS.primary.start}h)`
    )
  ...
  if (entryTime.getHours() > SECTOR_HOURS.extended.end) {
    warnings.push(
      `Entrada após o horário de funcionamento estendido (${SECTOR_HOURS.extended.end}h)`
    )
  ...
  if (entryTime.getHours() >= LUNCH_BREAK.start && entryTime.getHours() < LUNCH_BREAK.end) {
    warnings.push(
      `Entrada durante o intervalo de almoço (${LUNCH_BREAK.start}h-${LUNCH_BREAK.end}h)`
    )
...
export function validateExitTime(
  ...
  const isWithinWindow = isTimeWithinWindow(
    exitTimeStr,
    allowedExitWindow.start,
    allowedExitWindow.end
  )
  ...
  if (workedHours < expectedDailyHours - tolerance) {
    warnings.push(
      `Horas trabalhadas (${workedHours.toFixed(2)}h) abaixo do esperado (${expectedDailyHours.toFixed(2)}h)`
    )
  ...
  if (workedHours > expectedDailyHours + 2) {
    warnings.push(
      `Horas trabalhadas (${workedHours.toFixed(2)}h) acima do esperado (${expectedDailyHours.toFixed(2)}h)`
    )
