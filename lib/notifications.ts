...
export async function checkAndNotifyAttendance() {
    ...
    const interns = await (prisma as any).user.findMany({
        where: {
            role: 'EMPLOYEE',
        },
        include: {
    ...
    for (const intern of interns as any[]) {
        ...
        if (notificationType) {
            ...
            const alreadySent = intern.attendanceNotifications.some((n: any) => n.type === notificationType);
...
async function sendNotification(user: any, type: NotificationType, shiftEndTime: string) {
