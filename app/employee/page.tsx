...
export default function EmployeePage() {
  ...
  const processQrCode = async (qrData: string) => {
    ...
    // 🛡️ BLOQUEIO 2: Debounce - ignorar o mesmo QR code lido em menos de 2 segundos
    const now = Date.now()
    if (lastQrCode && lastQrCode.data === qrData) {
      const timeDiff = now - lastQrCode.timestamp
      if (timeDiff < 2000) {
