            ...
            <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="Ex: Bacharel em Engenharia"
            ...
            <CardContent className="p-6 space-y-4">
              <div>
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  placeholder="Expressar de forma concisa os pontos relevantes tratados no relatório..."
                  title="Resumo"
                />
              </div>
              <div>
          ...
          <Card variant="elevated">
            ...
            <CardContent className="p-6 space-y-4">
              <p className="text-xs text-neutral-500 italic">
                Descreva a estrutura da concedente, atividades realizadas, resultados alcançados,
                dificuldades e soluções encontradas.
              </p>
              <textarea
                name="desenvolvimento_text"
                rows={12}
                className="input w-full"
                value={formData.desenvolvimento_text || ''}
                onChange={handleInputChange}
                placeholder="Desenvolvimento do Relatório"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
                  {autoEvalItems.map((item) => (
                    ...
                    >
                      ...
                      <select
                        name={`eval_auto.${item.key}`}
                        className="bg-neutral-800 border-none rounded px-2 py-1 text-sm text-primary font-bold"
                        value={formData.eval_auto?.[item.key] || ''}
                        onChange={handleInputChange}
                        title={`Auto Avaliação: ${item.label}`}
                      >
                        <option value="">-</option>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
                  {supervisorEvalItems.map((item) => (
                    ...
                    >
                      <span className="text-sm text-neutral-300">{item.label}</span>
                      <select
                        name={`eval_supervisor.${item.key}`}
                        className="bg-neutral-800 border-none rounded px-2 py-1 text-sm text-primary font-bold"
                        value={formData.eval_supervisor?.[item.key] || ''}
                        onChange={handleInputChange}
                        title={`Avaliação da Supervisão: ${item.label}`}
                      >
                        <option value="">-</option>
                        <option value="insuficiente">Insuf.</option>
                        <option value="regular">Reg.</option>
            ...
            <CardContent className="p-6 space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  className="input w-full"
                  value={formData.consideracoes_finais || ''}
                  onChange={handleInputChange}
              ...
              <div>
                ...
                <textarea
                  name="consideracoes_matriz"
                  rows={4}
                  className="input w-full"
                  value={formData.consideracoes_matriz || ''}
                  onChange={handleInputChange}
                  placeholder="Citar disciplinas e conhecimentos teóricos aplicados..."
