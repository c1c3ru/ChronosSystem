...
function cleanPage(pageName) {
    ...
    const puppeteerPattern = /\/\/\s*Atualizar estado[\s\S]*?await generatePDFWithPuppeteer[\s\S]*?}\s*catch[\s\S]*?}\s*}/;
    content = content.replace(puppeteerPattern, '');

    // Remover template oculto (se existir)
    content = content.replace(/\s*{\/\*\s*Template Oculto[\s\S]*?\*\/}\s*<div[^>]*ref={templateRef}[\s\S]*?<\/div>/g, '');
