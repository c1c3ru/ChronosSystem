...
folders.forEach(folder => {
    ...
    const pattern1 = /if \(type === 'checkbox'\) \{\s+const checked = \(e\.target as HTMLInputElement\)\.checked\s+setFormData\(\(prev: any\) => \(\{ \.\.\.prev, \[name\]: checked \? value : '' \}\)\)\s+\} else \{/g;

    // Padrão 2: checkbox com 'true'/'false' (adicionar radio)
    const pattern2 = /if \(type === 'checkbox'\) \{\s+const checked = \(e\.target as HTMLInputElement\)\.checked\s+setFormData\(\(prev: any\) => \(\{ \.\.\.prev, \[name\]: checked \? 'true' : 'false' \}\)\)\s+\} else \{/g;

    // Padrão 3: sem tratamento de checkbox/radio (adicionar tudo)
    const pattern3 = /setFormData\(prev => \(\{ \.\.\.prev, \[name\]: maskedValue \}\)\)\s+\}/;
    const pattern3Alt = /setFormData\(\(prev: any\) => \(\{ \.\.\.prev, \[name\]: maskedValue \}\)\)\s+\}/;
